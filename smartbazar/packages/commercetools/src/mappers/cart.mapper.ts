...
export const mapOcCartToCart = (cart, lineItems): GetCartDto => {
  if (lineItems) {
    return {
...
export const mapCartToOcOrder = (cart: UpdateCartDto) => {
  return {
...
export const mapItemToOcLineItem = (item: AddCartItemDto) => {
  return {
    ID: item.id?.toString(),
...
export const mapItemToOcNewLineItem = (item: AddCartItemDto) => {
  return {

...
export default function CreateOrUpdateCatalogForm({ initialValues }: IProps) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="my-5 flex flex-wrap sm:my-8">
        <Description
          title={t('form:catalog-group')}
          details={t('form:catalog-group-help-text')}
          className="w-full px-0 pb-5 sm:w-4/12 sm:py-8 sm:pe-4 md:w-1/3 md:pe-5"
        />

        <Card className="w-full sm:w-8/12 md:w-2/3">
          <Input
            label={t('form:input-label-catalog-group')}
            {...register('settings.group')}
            error={t(errors.settings?.group?.message!)}
            variant="outline"
            className="mb-5"
          />
        </Card>
              ...
              >
                <div className="mb-5 flex items-center justify-between">
                  ...
                  <button
                    ...
                    type="button"
                    className="focus:outline-none text-sm text-red-500 transition-colors duration-200 hover:text-red-700 sm:col-span-1 sm:mt-4"
                  >

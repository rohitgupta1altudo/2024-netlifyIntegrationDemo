...
export function generateConfig(configOverrides?: { [key: string]: string }): void {
  ...
  const chatConfig = transformChatConfig();
  ...
  const config = Object.assign(defaultConfig, scjssConfig, packageJson,chatConfig, configOverrides);
...
function transformChatConfig() {
  // scjssconfig.json may not exist if you've never run `jss setup` (development)
  // or are depending on environment variables instead (production).
  let chatConfig;
  try {
    // eslint-disable-next-line global-require
    chatConfig = require('../chatconfig.json');
  } catch (e) {
    return {};
  }
  ...
  return {
    AzureStorageAccountEndpoint: chatConfig.chatConfigKeys.AzureStorageAccountEndpoint,
    AzureStorageContainer: chatConfig.chatConfigKeys.AzureStorageContainer,
  };
}

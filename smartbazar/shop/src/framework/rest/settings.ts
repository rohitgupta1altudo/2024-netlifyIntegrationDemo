import type { Settings, SettingsOptions } from '@/types';
...
export function useSettings() {
  const { queryCatalog } = useCatalog();
  const searchParams = queryCatalog ? { type: queryCatalog } : {};

  const { data, isLoading, error } = useQuery<Settings, Error>(
    [API_ENDPOINTS.SETTINGS, searchParams],
    ({ queryKey }) => client.settings.all(queryKey[1] as any)
  );

  return {
    settings: data?.options ?? ({} as SettingsOptions),
    isLoading,
    error,
  };
}

...
class Client {
  products = {
    ...
    getExact: (slugs: string) =>
      HttpClient.get<Product[]>(`${API_ENDPOINTS.PRODUCTS_EXACTLY}/${slugs}`),
  };
  cart = {
    ...
    }) => HttpClient.put(API_ENDPOINTS.CART, input),
    submit: () => HttpClient.post<Order>(API_ENDPOINTS.CART, {}),
    addItem: (input: CartItem) =>
      HttpClient.post(`${API_ENDPOINTS.CART}/items`, input),
    updateItem: (variables: { id: string | number; input: CartItem }) =>
      HttpClient.put(
        `${API_ENDPOINTS.CART}/items/${variables.id}`,
        variables.input
      ),
    deleteItem: (id: string) =>
      HttpClient.delete(`${API_ENDPOINTS.CART}/items/${id}`),
    clearCart: () => HttpClient.delete<Order>(API_ENDPOINTS.CART),
  };
  categories = {
    ...
    get: ({ type, category }: Partial<SingleCategoryQueryOptions>) =>
      HttpClient.get<Category>(
        `${API_ENDPOINTS.USERS_CATALOGS}/${type}/${category}`
      ),
  };
  tags = {
  ...
  orders = {
    all: (params: Partial<OrderQueryOptions>) =>
      HttpClient.get<OrderPaginator>(API_ENDPOINTS.USERS_ORDERS, params),
    get: (tracking_number: string) =>
      HttpClient.get<Order>(`${API_ENDPOINTS.USERS_ORDERS}/${tracking_number}`),
    create: (input: CreateOrderInput) =>
      HttpClient.post<Order>(API_ENDPOINTS.USERS_ORDERS, input),
  ...
  users = {
    ...
    updateAddress: (user: AddressInput) =>
      HttpClient.put<User>(`${API_ENDPOINTS.USERS_ME_ADDRESS}`, user),
    ...
    deleteAddress: ({ id }: { id: string }) =>
      HttpClient.delete<boolean>(`${API_ENDPOINTS.USERS_ME_ADDRESS}/${id}`),
  ...
  settings = {
    ...
    all: async ({ type, ...params }: Partial<CategoryQueryOptions>) =>
      HttpClient.get<Settings>(API_ENDPOINTS.SETTINGS, {
        searchJoin: 'and',
        ...params,
        ...(type && { search: HttpClient.formatSearchParams({ type }) }),
      }),
  ...
  aichat = {
    getChatResponse : (chatRequestDto: AICHATREQUEST ) =>
      HttpClient.post<BotResponse>(API_ENDPOINTS.AICHAT, chatRequestDto)
  };
}

...
const GraphQLConnectedDemo = (
  props: GraphQLConnectedDemoProps
): JSX.Element => {
  useEffect(() => {
    resetEditorChromes();
  }, []);
  ...
  return (
    <div data-e2e-id="graphql-connected">
      ...
      <p>
        ...
        GraphQL endpoint. This example runs the query server-side using
        component-level <code>getStaticProps</code>/
        <code>getServerSideProps</code>, a feature of the Sitecore JSS Next.js
        SDK. These are aggregated during the the Next.js page-level{' '}
      ...
      {props.datasource && (
        <div>
          ...
          <br />
          sample1 (editable):{' '}
          <Text field={props.datasource.sample1?.jsonValue} />
          <br />
          sample2:
          <br />
          <ul>
            ...
            <li>
              field is shared?:{' '}
              {props.datasource.sample2?.definition?.shared.toString()}
            </li>
          </ul>
        </div>
      )}
        ...
        <div>
          ...
          <ul>
            {props.contextItem.children.results.map((child) => {
              ...
              return (
                <li key={routeItem.id}>
                  <NextLink href={routeItem.url.path}>
                    {routeItem.pageTitle?.value}
                  </NextLink>
                  (editable title too!{' '}
                  <Text field={routeItem.pageTitle?.jsonValue} />)
                </li>
              );
            })}
          </ul>
